...
from comparador.models import TotalizacaoPartido, ResultadoSimulacaoCorrigida, TotalizacaoCandidato
from django.db.models import Sum
from django.conf import settings
from decimal import Decimal
from django.db.models import F

# Classificação dos Eleitores pela Lógica Corrigida
class Command(BaseCommand):
    def handle(self, *args, **options):
        
        partidos = TotalizacaoPartido.objects.all()
        ResultadoSimulacaoCorrigida.objects.all().delete() #limpa para começar de novo
        #reinicia as vagas obtidas 
        for partido in partidos:
            partido.vagas_obtidas_algoritmo_corrigido = partido.qp
            partido.save()
        ...
        for partido in partidos:
            ...
            for c in candidatospartido:
                ResultadoSimulacaoCorrigida.objects.get_or_create(
                        cd_cargo = c.cd_cargo,
                        ds_cargo = c.ds_cargo,
        ...
        vagas_remanescentes = int(settings.VAGAS_DEPUTADO_2022 - TotalizacaoPartido.objects.all().aggregate(Sum('vagas_obtidas_algoritmo_corrigido'))['vagas_obtidas_algoritmo_corrigido__sum'])
        #print (vagas_remanescentes)
        
        partidos_ignorar = []      

        #Calculo de sobras (Considerando art. 109, inciso I do Código Eleitoral)
        for i in range(1,vagas_remanescentes + 1):
            #print(i)
            #traz o partido com melhor quociente excluindo-se os que não tem candidatos que atingiram os 20% e apenas os partidos que atingiram o quociente eleitoral 
            partido = TotalizacaoPartido.objects.filter(qt_votos_validos__gte=qe80, vagas_obtidas_algoritmo_corrigido__gte=1).annotate(votacao_vagas=F('qt_votos_validos')/(F('vagas_obtidas_algoritmo_corrigido') + 1)).exclude(ds_composicao_coligacao__in=partidos_ignorar).order_by('-votacao_vagas')[0]

            #obtem os candidatos eleitos para excluir da distribuição
            candidatos_eleitos = ResultadoSimulacaoCorrigida.objects.all().values_list('nr_candidato', flat=True)
            ...
            if partido:
                ...
                if candidatospartido:
                    ...
                    # diminui uma vaga
                    partido.vagas_obtidas_algoritmo_corrigido += 1 
                    partido.save()
                    #print (vagas_remanescentes)
                else:
